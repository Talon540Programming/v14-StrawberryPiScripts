...
thresholds = [(30, 80, 34, 127, 18, 108), # generic_red_thresholds -> index is 0 so code == (1 << 0)
              (11, 81, -26, 43, -83, -34)] # generic_blue_thresholds -> index is 2 so code == (1 << 2)
...
while(True):
    ...
    for blob in img.find_blobs(thresholds, pixels_threshold=100, area_threshold=100, merge=False):
        if blob.roundness() >.75:
            if blob.code() == 1: # r code
                img.draw_rectangle(blob.rect())
                img.draw_cross(blob.cx(), blob.cy())
                text  = "r - "+str(blob.roundness())
                img.draw_string(blob.x() + 2, blob.y() + 2, text)
            if blob.code() == 2: # r/g code
                img.draw_rectangle(blob.rect())
                img.draw_cross(blob.cx(), blob.cy())
                text  = "b - "+str(blob.roundness())
                img.draw_string(blob.x() + 2, blob.y() + 2, text)
