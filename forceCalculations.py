...
def shooterCalculations(distanceFromTopHub):
    ...
    d = math.sqrt(math.pow(distanceFromTopHub, 2) - math.pow((hubHeight - limeLightHeight),
                                                             2)) + 0.2  # Distance from hub base to shooter
    ...
    shootingSpeed = math.sqrt(-((
                9.81 * math.pow(d, 2) * (1 + (math.pow((math.tan(math.radians(shootingAngle))), 2))) / (
                    (2 * H) - (2 * d * math.tan(math.radians(shootingAngle)))))))
